...
class datahandler(Configuration):
    def __init__(self) -> None:
        ...
        client = pymongo.MongoClient("mongodb+srv://puter:q1w2e3r4t5>@cluster0.v2kpx.mongodb.net/crypto?retryWrites=true&w=majority")
        db = client.raw
    ...
    def getScrip(self, scrip=None, interval='1m', From='2 days ago UTC', update=False, save=False): 
        ...
        if save: df.to_csv(file, index=False, header=None)
    ...
    def Grouper(self, df, freq, aggVars=None, key='openTime'):
        ...
        df = df.groupby(pd.Grouper(key=key, freq=freq)).agg(self.aggVars).reset_index(drop=True)
