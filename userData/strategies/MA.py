...
class MA(IStrategy):
    ...
    def populate_indicators(dataframe: DataFrame, metadata=None) -> DataFrame:
        ...
        dataframe["SLOWMA"] = F.EMA(dataframe, 6)
        dataframe["FASTMA"] = F.TEMA(dataframe, 6)
        dataframe["SupportMA"] = F.SMA(dataframe, 50)
        if 'Close' in dataframe.keys(): dataframe['close'] = dataframe['Close']
